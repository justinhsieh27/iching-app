...
const drawReport = (ctx, { lines, question, interpretation, hexKey, aiResult, t, i18n }, dryRun) => {
    ...
    const drawSectionTitle = (title, currentY) => {
        if (!dryRun) {
            ctx.font = 'bold 22px "Noto Serif TC", "Songti TC", "Georgia", "PMingLiU", serif';
            ctx.fillStyle = '#2b2621';
            
            // Draw a left accent bar
            ctx.fillStyle = '#8d6e63';
            ctx.beginPath();
            drawRoundRect(ctx, padding, currentY - 18, 4, 22, 2);
            ctx.fill();
            
            ctx.fillStyle = '#2b2621';
            ctx.fillText(title, padding + 15, currentY);
        }
        return currentY + 35;
    };
    ...
    // --- 1. HEADER ---
    if (!dryRun) {
        ctx.font = 'bold 36px "Noto Serif TC", "Songti TC", "Georgia", "PMingLiU", serif';
        ctx.fillStyle = '#2b2621';
    ...
    if (!dryRun) {
        ctx.font = 'italic 16px "PingFang SC", "Microsoft JhengHei", "Noto Sans TC", sans-serif';
        ctx.fillStyle = '#8d6e63';
        ctx.textAlign = 'center';
    ...
    // --- 2. QUESTION ---
    if (question) {
        const questionBoxPadding = 20;
        const questionTextSize = 18;
        ...
        const boxHeight = questionHeight + 50; // padding top/bottom and label

        if (!dryRun) {
            // Draw box
            ctx.fillStyle = '#f5efe6';
            ctx.beginPath();
            drawRoundRect(ctx, padding, y, contentWidth, boxHeight, 8);
            ctx.fill();

            // Draw label
            ctx.font = 'bold 13px "PingFang SC", sans-serif';
            ctx.fillStyle = '#8c7e6c';
            ctx.fillText(i18n.language === 'en' ? 'QUESTION ASKED' : '所問之事', padding + questionBoxPadding, y + 25);
        }
    ...
    if (!dryRun) {
        ctx.font = '16px "PingFang SC", "Microsoft JhengHei", sans-serif';
    ...
    if (!dryRun) {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            ctx.fillStyle = '#a69b8d';
            ctx.textAlign = 'right';
            ctx.fillText(String(i + 1), startX - 20, lineY + 11);
            ctx.textAlign = 'left';
            ...
            if (isYang) {
                // Yang line (solid)
                ctx.beginPath();
                drawRoundRect(ctx, startX, lineY, lineLength, lineThickness, 3);
                ctx.fill();
            } else {
                // Yin line (broken)
                ...
                ctx.beginPath();
                drawRoundRect(ctx, startX, lineY, segmentLength, lineThickness, 3);
                ctx.fill();
                
                ctx.beginPath();
                drawRoundRect(ctx, startX + segmentLength + gap, lineY, segmentLength, lineThickness, 3);
                ctx.fill();
            }
            ...
            if (isChanging) {
                ctx.beginPath();
                ctx.arc(startX + lineLength + 25, lineY + (lineThickness / 2), 6, 0, Math.PI * 2);
    ...
    if (interpretation) {
        ...
        y += 24;
        y = drawWrappedText(interpretation.keywords || '', y, 16, '#2b2621', '', 1.6);
        y += 15;
        ...
        y += 24;
        y = drawWrappedText(interpretation.coreMeaning || '', y, 16, '#2b2621', '', 1.65);
        y += 20;
        ...
        if (interpretation.aspects) {
            ...
            for (let i = 0; i < aspectsList.length; i++) {
                ...
                if (aspectText) {
                    ...
                    y += 22;
                    y = drawWrappedText(aspectText, y, 15, '#5e5349', '', 1.55);
                    y += 10;
                }
            }
        }
        ...
        if (Array.isArray(interpretation.lines) && interpretation.lines.length > 0) {
            ...
            for (let i = 0; i < interpretation.lines.length; i++) {
                ...
                // Line Meaning
                y = drawWrappedText(lineData.meaning, y, 15, '#5e5349', '', 1.55);
                y += 15;
            }
        }
    }
    ...
    if (!dryRun) {
        ctx.font = 'italic 16px "Noto Serif TC", "Songti TC", "Georgia", "PMingLiU", serif';
        ctx.fillStyle = '#8d6e63';
        ctx.textAlign = 'center';
    ...
    if (!dryRun) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        drawRoundRect(ctx, 20, 20, width - 40, y + 40 - 40, 4);
        ctx.stroke();

        ctx.lineWidth = 2;
        ctx.beginPath();
        drawRoundRect(ctx, 26, 26, width - 52, y + 40 - 52, 4);
        ctx.stroke();
